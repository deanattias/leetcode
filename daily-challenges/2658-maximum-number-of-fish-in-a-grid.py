class Solution:
    # Helper function to count the number of fish in a connected component
    def calculate_fish(self, grid, visited, row, col):
        ...
        return (
            grid[row][col]
            + self.calculate_fish(grid, visited, row, col + 1)
            + self.calculate_fish(grid, visited, row, col - 1)
            + self.calculate_fish(grid, visited, row + 1, col)
            + self.calculate_fish(grid, visited, row - 1, col)
        )
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if grid[row][col] > 0 and not visited[row][col]:
                    max_fish_count = max(
                        max_fish_count,
                        self.calculate_fish(grid, visited, row, col),
                    )
